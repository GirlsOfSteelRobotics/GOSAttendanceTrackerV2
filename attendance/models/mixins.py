...
class InOutTimeMixin:
    ...
    def num_hours_today_hm(self) -> str:
        ...
        attendance_query = self._get_attendance_set().filter(
            models.Q(time_in__gte=start_of_day) | models.Q(time_out__gte=start_of_day) | models.Q(time_out__isnull=True)
        ).distinct()
        ...
        for x in attendance_query:
            ...
            is_stale = x.time_out is None and (now - x.time_in) > datetime.timedelta(hours=18)
            if is_stale:
                continue
    ...
    def num_hours_this_week_hm(self) -> str:
        ...
        attendance_query = self._get_attendance_set().filter(
            models.Q(time_in__gte=start_of_week) | models.Q(time_out__gte=start_of_week) | models.Q(time_out__isnull=True)
        ).distinct()
